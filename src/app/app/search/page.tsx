  ...
  return (
    <div className="content-stack">
      <section className="card search-card">
        <form><input name="q" defaultValue={q} placeholder="Search imported messages, events, and people…" aria-label="Search Compass"/><button className="button primary">Search</button></form>
      </section>
      {!q && (
      ...
      {q && (
        <section className="card">
          <div className="section-heading"><h2>{total} results</h2><span className="pill" title="These results are visible only in your private profile">Private search</span></div>
          <div className="search-results">
            {messages.map(item => <Result key={`m-${item.id}`} href={`/app/messages?message=${item.id}`} icon="✉" title={item.subject || item.sender || "Message"} detail={item.preview || new Date(item.occurred_at).toLocaleString()}/>) }
            {events.map(item => <Result key={`e-${item.id}`} href={`/app/calendar?event=${item.id}`} icon="◫" title={item.title} detail={`${new Date(item.starts_at).toLocaleString()}${item.location ? ` • ${item.location}` : ""}`}/>) }
            {people.map(item => <Result key={`p-${item.id}`} href={`/app/people/${item.id}`} icon="◎" title={item.display_name} detail={(item.email_addresses || []).join(", ") || (item.phone_numbers || []).join(", ") || "Open contact"}/>) }
...
function Result({ href, icon, title, detail }: { href: string; icon: string; title: string; detail: string }) {
  return <Link className="search-result selectable-row" href={href}><span>{icon}</span><div><b>{title}</b><p>{detail}</p></div><span className="row-chevron" aria-hidden="true">›</span></Link>;
}
