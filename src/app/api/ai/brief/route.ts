...
function sourceLabel(provider: string, connectionId: string | null, accounts: Map<string, AccountRow>): string {
  ...
  const displayProvider = providerLabel(account?.provider || provider);
  return account?.account_email ? `${displayProvider} · ${account.account_email}` : displayProvider;
}
...
function localBrief(context: BriefContext): Brief {
  ...
  const dueSoon = context.tasks.filter(task => task.due_at && Date.parse(task.due_at) <= now + day).slice(0, 5).map(task => `${Date.parse(task.due_at || "") < now ? "Overdue" : "Due soon"}: ${task.title}`);
  const accountIssues = context.accountHealth.filter(account => !["healthy", "syncing"].includes(account.status)).slice(0, Math.max(0, 5 - dueSoon.length)).map(account => `${account.source}: ${account.status.replace("_", " ")}`);
  const attentionMessages = context.messages.filter(message => urgentKeywords.test(`${message.subject || ""} ${message.preview || ""}`)).slice(0, Math.max(0, 5 - dueSoon.length - accountIssues.length)).map(message => `${message.source}: ${message.subject || message.sender || "Needs review"}`);
  const upcoming = context.events.slice(0, 6).map(event => `${event.title} — ${new Date(event.startsAt).toLocaleString()}${event.location ? ` at ${event.location}` : ""} · ${event.source}`);
  ...
  return { headline: `${openTasks.length} open follow-up${openTasks.length === 1 ? "" : "s"}, ${todayEvents} event${todayEvents === 1 ? "" : "s"} in the next day, ${recentMessages} recent message${recentMessages === 1 ? "" : "s"}${unhealthyAccounts ? `, and ${unhealthyAccounts} account issue${unhealthyAccounts === 1 ? "" : "s"}` : ""}.`, urgent: [...dueSoon, ...accountIssues, ...attentionMessages], schedule: upcoming, followUps: openTasks };
}

async function aiBrief(context: BriefContext): Promise<{ brief: Brief; model: string }> {
  const client = openAiClient();
  const model = env.openAiModel();
  const response = await client.responses.create({ model, store: false, input: [{ role: "developer", content: "Create a concise executive operating brief from the supplied private CompassOS context. Never invent facts, obligations, urgency, people, dates, or account state. Preserve provider/account identity when it clarifies where an item lives. Prioritize genuine exceptions, deadlines, account failures, near-term schedule, and concrete follow-ups. Do not call this a household brief unless the supplied data itself is explicitly shared household data." }, { role: "user", content: JSON.stringify(context) }], text: { format: { type: "json_schema", name: "compass_executive_brief", strict: true, schema: { type: "object", additionalProperties: false, required: ["headline", "urgent", "schedule", "followUps"], properties: { headline: { type: "string" }, urgent: { type: "array", items: { type: "string" } }, schedule: { type: "array", items: { type: "string" } }, followUps: { type: "array", items: { type: "string" } } } } } } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const result of [messagesResult, eventsResult, tasksResult, accountsResult]) if (result.error) throw new Error("BRIEF_SOURCE_QUERY_FAILED");
    const context = buildContext((messagesResult.data || []) as MessageRow[], (eventsResult.data || []) as EventRow[], (tasksResult.data || []) as TaskRow[], (accountsResult.data || []) as AccountRow[]);
    let brief = localBrief(context);
    let model = "compass-local-v2";
    let mode: "local" | "ai" = "local";
    if (process.env.OPENAI_API_KEY) {
      try { const generated = await aiBrief(context); brief = generated.brief; model = generated.model; mode = "ai"; }
      catch (aiError) { console.error("Compass AI executive brief fell back to local mode", aiError instanceof Error ? aiError.message : aiError); }
    }
